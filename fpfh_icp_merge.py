...
point_cloud_poses = {
    "1.txt": [-0.04375, -0.16451, 0.79252, 0.884, 2.629, -0.264],
    "2.txt": [-0.06126, -0.11841, 0.73504, 0.692, 2.617, -0.429],
    "3.txt": [-0.06929, -0.04890, 0.55885, 1.003, 2.105, -0.293],
    "4.txt": [-0.14677, -0.17205, 0.56815, 2.180, -0.579, 1.668]
}
inputs = "point_clouds"
outputs = "output"
target_name = "point_clouds\\merged_all_clouds2.txt"
# 2. 点云文件存放目录（必填）
PCD_DIRECTORY = "./point_clouds"  # 示例：所有TXT点云放在该文件夹下

# 3. 配准参数（可选，根据点云精度调整）
VOXEL_SIZE = 0.5  # 降采样体素大小（单位：毫米）
ICP_MAX_DISTANCE = 5.0  # ICP最大对应点距离（单位：毫米）
...
def merge_point_clouds(rel_path,target_pcd):
        # 加载源点云
        print(rel_path)
        source_pcd = read_point_cloud(rel_path)
        ...
        fpfh_result = coarse_registration(source_pcd, target_pcd)
        # 2. 仅对source_init应用粗配准的微调变换（而非对原始source_pcd叠加变换）
        source_fpfh_aligned = copy_point_cloud(source_pcd)
        ...
        # o3d.visualization.draw_geometries(
        #     [target_pcd, source_fpfh_aligned, o3d.geometry.TriangleMesh.create_coordinate_frame(size=50)],
        #     window_name="FPFH粗配准后"
        # )
        ...
        # )
        return source_transformed_fpfh
        # # 可视化当前配准结果
        # visualize_result(
...
def batch_process(input_root, output_root,merged_pcd,target_pcd):
    """
    ...
    for idx, in_path in enumerate(txt_files, 1):
        ...
        # else:
        #     continue
        pcd = merge_point_clouds(in_path,merged_pcd)
        merged_pcd+=pcd
    ...
    txt_path = os.path.join(PCD_DIRECTORY, "3.txt")
...
def merge_all_point_clouds(input,out,target_name):
    """融合所有点云到目标点云坐标系"""
    # 加载目标点云
    target_pcd = read_point_cloud(target_name)
    ...
    batch_process(input,out,merged_pcd,target_pcd)
    # # 遍历所有源点云
    ...
    #
    #     # 加载源点云
    #     source_pcd = read_point_cloud(source_name)
...
def main():
    ...
    # 融合所有点云（以第一组为目标）
    merge_all_point_clouds(inputs,outputs,target_name)
